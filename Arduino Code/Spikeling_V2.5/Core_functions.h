...
inline void update_NeuronMode(){
  neuron.modeState = digitalRead(pins.gpio.mode);                                             // Read the mode button state
  if (neuron.modeState == HIGH){                                                               // If the mode button is pushed:
    neuron.mode += 1;                                                                           // Increment the Mode by 1
    if (neuron.mode >= neuron.nModes){                                                          // If Mode has reached the maximum number of existing modes:
      neuron.mode = 0;                                                                            // Reset Mode to 0
    }
    lightOn(neuron.mode);                                                                       // Turn next Mode_LED
    
    auto model = clampToModel(static_cast<std::size_t>(neuron.mode));
    SetNeuronModel(model);

    delay(neuron.modeDelay);                            // Set a delay to prevent multi_push from the user
  }
}
...
/*                                 --  All Stimulus GUI buttons OFF  --                                  */
  ...
  if ( stim.custom_enable ){                                                                  // If GUI "Custom Stimulus" is disabled:
    ...
    int val = static_cast<int>(stim.value_analog);                                            // Quantize/clamp to 8-bit range
    val = constrain(val, 0, dac_max);
    stim.dac = static_cast<uint8_t>(val);

    ledcWrite(stim.pin_stim_light, stim.pwm);                                                 // Stimulating LED
    dacWrite(stim.pin_stim_current, stim.dac);                                                // Stimulus current output
...
/*                               --  GUI Custom Stimulus button is  OFF --                               */
  ...
  if ( stim.custom_enable ){                                                                  // If GUI "Custom Stimulus" button is disabled: 
    IC.value_currentIn = ADC2.read(IC.pin);                                                     // Reads Current-in value
