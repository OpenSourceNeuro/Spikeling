...
const float SYN_V_SCALE = 100.0f;                           // 2 decimal places for voltages
...
inline MCP4922 DAC;                                         // MCP4922 12-bit SPI DAC
...
struct MCP3208_pins {
  ...
  uint8_t stim_freq_pot;                                    // ADC 2.0: Stimulus frequency potentiometer pin
  uint8_t stim_str_pot;                                     // ADC 2.1: Stimulus amplitude potentiometer pin
  uint8_t noise_pot;                                        // ADC 2.2: Noise generator potentiometer pin                                                   
};  
...
struct GPIO_pins {
  ...
  uint8_t led_stim_r;                                       // Red Stimulus LED
  uint8_t led_stim_g;                                       // Green Stimulus LED
  uint8_t led_stim_b;                                       // Blue Stimulus LED
  uint8_t buzzer_button;                                    // Buzzer switch button
  uint8_t led_button;                                       // LED switch button
};
...
struct HardwarePins {
  ...
  MCP4922_pins    dac;                                      // MCP4922 channels
...
inline ButtonDebounce buzzerButton {
  .lastStable         = LOW,                                // Debounced stable level
  .lastRaw            = LOW,                                // Last raw read level
  .lastChange_ms      = 0,                                  // Timestamp of last raw transition
  .debounce_ms        = 30,                                 // Debounce window (ms)
  .raw                = LOW,                                // Latest sampled raw level (filled by poll)
  .now_ms             = 0                                   // Latest sampled time (filled by poll)
};
inline ButtonDebounce ledButton {
  .lastStable         = LOW,                                // Debounced stable level
  .lastRaw            = LOW,                                // Last raw read level
  .lastChange_ms      = 0,                                  // Timestamp of last raw transition
  .debounce_ms        = 30,                                 // Debounce window (ms)
...
struct Axon {
  ...
  uint16_t dacVal;                                          // Quantized 12-bit DAC code written to pin_analog (0..dac_max)
}; 
inline Axon axon{
...
// // // // // // // // // // // // // // // // // // // // // // // //
/*                    Buzzer / LED enable helpers                    */

inline void setBuzzerEnabled(bool enable) {
  spike.Buzzer_enable = enable;                             // Store the requested buzzer state
  if (!spike.Buzzer_enable) {                               // If we just disabled the buzzer:
    digitalWrite(pins.gpio.spike, LOW);                     // Force the buzzer GPIO LOW immediately (avoid stuck HIGH)
  }
}

inline void toggleBuzzerEnabled() {
  setBuzzerEnabled(!spike.Buzzer_enable);                   // Flip enable flag + apply hardware side-effect
}

inline void setLedEnabled(bool enable) {
  spike.LED_enable = enable;                                // Store the requested LED state
  if (!spike.LED_enable) {                                  // If we just disabled the RGB LEDs:
    ledcWrite(pins.gpio.led_r, 0);                          // Force Red PWM to 0
    ledcWrite(pins.gpio.led_g, 0);                          // Force Green PWM to 0
    ledcWrite(pins.gpio.led_b, 0);                          // Force Blue PWM to 0
    ledcWrite(pins.gpio.led_stim_r, 0);                     // Force stim Red OFF when LEDs disabled
    ledcWrite(pins.gpio.led_stim_g, 0);                     // Force stim Green OFF when LEDs disabled
    ledcWrite(pins.gpio.led_stim_b, 0);                     // Force stim Blue OFF when LEDs disabled
    spike.led_r_last = 0;                                   // Reset cached last duty (so re-enable resumes correctly)
    spike.led_g_last = 0;                                   // Reset cached last duty (so re-enable resumes correctly)
    spike.led_b_last = 0;                                   // Reset cached last duty (so re-enable resumes correctly)
  }
}

inline void toggleLedEnabled() {
  setLedEnabled(!spike.LED_enable);                         // Flip enable flag + apply hardware side-effect
}
...
inline void init_PotFilters() {
  ...
  noise.pot_filt.reset(ADC2.read(noise.pot_pin));           // Seed filter with a real reading
  ...
  stim.strPot_filt.reset(ADC2.read(stim.pin_strPot));       // Seed filter with a real reading
  ...
  stim.freqPot_filt.reset(ADC2.read(stim.pin_freqPot));     // Seed filter with a real reading
}
...
inline void HardwareSettings(){
  ...
  pinMode(pins.spi.cs_dac,  OUTPUT); digitalWrite(pins.spi.cs_dac,  HIGH);
  // Start SPI 
